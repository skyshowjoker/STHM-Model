...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    for preprocessed in preprocessing:
        ...
        # data_img = d[0, 8, :, :].astype(np.uint8)
        # pred_img = softmax[0, 0, 8, :, :].astype(np.uint8)
        # img = cv2.applyColorMap(data_img, cv2.COLORMAP_JET)
...
def pre(in_dir, out_dir):
    ...
    parser.add_argument('-m', '--model_output_folder', default='3d_fullres',
    ...
    model = r'C:\Users\perlicue\PycharmProjects\MHM-Model\aggregate\seg_model'
    folds = args.folds
