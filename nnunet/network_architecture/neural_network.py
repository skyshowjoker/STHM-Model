...
class SegmentationNetwork(NeuralNetwork):
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        # class_probabilities *= 255
        # # class_probabilities = 255 - class_probabilities
        # img = (class_probabilities[5, 10, :, :]+class_probabilities[12, 10, :, :]).astype(np.uint8)
        # cv_data = data[0, 0, :, :].astype(np.uint8)
        # # cv2.imwrite(r"C:\Users\perlicue\Desktop\cv_data.jpg", cv_data)
        # # cv2.namedWindow("Image")
        # # img = cv2.applyColorMap(img, cv2.COLORMAP_JET)
        # # cv2.imshow("Image", img)
        # # img = cv2.equalizeHist(img)
        # # img = cv2.GaussianBlur(img, (501, 501), 5)
        ...
        # img = cv2.applyColorMap(img, cv2.COLORMAP_JET)
        # cv2.imshow("Image", img)
        # cv2.imwrite(r"C:\Users\perlicue\Desktop\cv_save.jpg", img)
        # cv2.waitKey(0)
